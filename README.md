...
| `common.ssl.secret`                                   | Name of a Secret to be generated by `deploy-cortx-cloud.sh` that contains a self-signed certificate.<br />Note: Either `common.ssl.secret` or `common.ssl.external_secret` must be specified. | `""` |
| `common.ssl.external_secret`                          | Names of an existing Secret that contains a TLS/SSL certificate.  This certificate must be in PEM format and must contain the Private Key and Certificate.<br />Note: Either `common.ssl.secret` or `common.ssl.external_secret` must be specified. | `""` |
| `common.ssl.secret_key`                               | The Secret key for the field that contains the certificate. | `""` |
