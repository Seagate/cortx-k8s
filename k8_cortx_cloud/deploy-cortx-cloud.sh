...
function deployCortxConfigMap()
{
    ...
    for idx in "${!node_name_list[@]}"; do
        helm_install_args+=(
            ...
            --set "cortxRgw.rgwServiceHttpEndpoints[${idx}]=tcp://cortx-server-headless-svc-${node_name_list[${idx}]}:22751"
            --set "cortxRgw.rgwServiceHttpsEndpoints[${idx}]=tcp://cortx-server-headless-svc-${node_name_list[${idx}]}:23001"
        )
    done
