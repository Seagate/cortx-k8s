...
| existingCertificateSecret | string | `""` | The name of an existing Secret that contains the CORTX SSL Certificate secret.  The key must be `cortx.pem`.  The value must be in PEM format and contain the Private Key and Certificate.  If not specified, a self-signed certificate will be generated. |
